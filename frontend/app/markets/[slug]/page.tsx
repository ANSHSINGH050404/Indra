...
export default function MarketDetailPage() {
    ...
    const fetchData = async () => {
      try {
        const [marketRes, posRes, historyRes, commentsRes] = await Promise.all([
          api.get(`/api/markets/${slug}`).catch((e) => {
            throw e;
          }), // Required
          getPositions().catch(() => []), // Optional
          api.get(`/api/markets/${slug}/history`).catch(() => ({ data: [] })), // Optional
          api.get(`/api/comments/${slug}`).catch(() => ({ data: [] })), // Optional
        ]);
        ...
        const yesHist =
          (historyRes.data || []).find(
            (h: any) => h.outcomeTitle?.toUpperCase() === "YES",
          )?.history || [];
        const noHist =
          (historyRes.data || []).find(
            (h: any) => h.outcomeTitle?.toUpperCase() === "NO",
          )?.history || [];
        ...
        const yesOutcome = marketRes.data?.outcomes?.find(
          (o: any) => o.title?.toUpperCase() === "YES",
        );
        const noOutcome = marketRes.data?.outcomes?.find(
          (o: any) => o.title?.toUpperCase() === "NO",
        );
        if (yesOutcome) {
          merged.push({
            time: "Now",
  ...
  const handleTrade = async (title: "YES" | "NO") => {
    ...
    const toastId = toast.loading(
      `${tradeType === "BUY" ? "Buying" : "Selling"} shares...`,
    );
    try {
      if (tradeType === "BUY") {
            ...
            <div className="bg-[#0d0f17] border border-white/5 rounded-3xl p-8 md:p-12 shadow-2xl">
              ...
              <div className="flex flex-wrap gap-8">
                ...
                <div>
                  ...
                  <p className="font-bold capitalize text-emerald-400">
                    {market.status}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-[#111318] border border-white/5 rounded-3xl p-8 sticky top-24">
              ...
              <div className="mb-10">
                ...
                <div className="relative">
                  {tradeType === "BUY" && (
                    <span className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-400 font-bold text-xl">
                      ₹
                    </span>
                  )}
                  <input
                  ...
                  <div className="mt-4 flex flex-col gap-1 text-[10px] font-bold uppercase tracking-wider text-zinc-500 bg-white/5 p-4 rounded-xl">
                    <div className="flex justify-between">
                      ...
                      <span className="text-emerald-400">
                        {getHoldingForOutcome(
                          market.outcomes?.find((o) => o.title === "YES")?.id ||
                            "",
                        )}{" "}
                        shares
                      </span>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-red-400">
                        {getHoldingForOutcome(
                          market.outcomes?.find((o) => o.title === "NO")?.id ||
                            "",
                        )}{" "}
                        shares
                      </span>
                ...
                <div className="space-y-4">
                  <button
                    ...
                    disabled={
                      ...
                      (tradeType === "SELL" &&
                        amount >
                          getHoldingForOutcome(
                            market.outcomes.find((o) => o.title === "YES")
                              ?.id || "",
                          ))
                    }
                  ...
                  >
                    ...
                    {tradeType === "BUY" && (
                      <span className="text-[10px] mt-2 opacity-60">
                        @ ₹
                        {market.outcomes.find((o) => o.title === "YES")?.price}{" "}
                        per share
                      </span>
                    )}
                  </button>
                  ...
                  >
                    ...
                    {tradeType === "BUY" && (
                      <span className="text-[10px] mt-2 opacity-60">
                        @ ₹
                        {market.outcomes.find((o) => o.title === "NO")?.price}{" "}
                        per share
                      </span>
                    )}
                  </button>
              ...
              {status && (
                <div
                  className={`mt-8 p-5 rounded-2xl border text-center font-bold animate-in zoom-in duration-300 ${
                    status.type === "success"
